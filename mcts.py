...
class MCTS:
    ...
    def find_leaf(self, state_int, player):
        ...
        while not self.is_leaf(cur_state):
            ...
            # check for the draw
            if value is None and len(s.get_action_space(cur_state)) == 0:
                value = 0.0
    ...
    def search_minibatch(self, count, state_int, player, net, device="cpu"):
        ...
        if expand_queue:
            batch_v = torch.FloatTensor([s.state_to_batch(state, player) for state, player in zip(expand_states, expand_players)])
            ...
            for (leaf_state, states, actions), value, prob in zip(expand_queue, values, probs):
                self.visit_count[leaf_state] = [0] * 9
                self.value[leaf_state] = [0.0] * 9
                self.value_avg[leaf_state] = [0.0] * 9
